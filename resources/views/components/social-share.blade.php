...
@php
    ...
    $shareTitle = $title ?: config('company.name');
    $shareDescription = $description ?: config('company.tagline');
    $encodedUrl = rawurlencode($shareUrl);
    $encodedTitle = rawurlencode($shareTitle);
    $encodedText = rawurlencode($shareTitle.' — '.$shareDescription);
    $instagramProfile = config('company.social.instagram');
@endphp

<div {{ $attributes->merge(['class' => 'social-share flex flex-wrap items-center gap-2 '.$class]) }} data-share-root>
    <span class="text-sm font-semibold text-gray-700 mr-1">Share</span>

    <a href="https://www.facebook.com/sharer/sharer.php?u={{ $encodedUrl }}"
       target="_blank"
       rel="noopener noreferrer"
       aria-label="Share on Facebook"
       class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-600 transition hover:border-orange-400 hover:bg-orange-50 hover:text-orange-600">
    ...
    <button type="button"
            ...
            data-ig-profile="{{ $instagramProfile }}"
    ...
    <button type="button"
            ...
            title="Copy link"
            class="inline-flex h-9 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-xs font-semibold text-gray-600 transition hover:border-orange-400 hover:bg-orange-50 hover:text-orange-600">
...
document.addEventListener('click', async function (event) {
    const copyBtn = event.target.closest('[data-copy-share], [data-ig-share]');
    if (!copyBtn) return;
    ...
    try {
        ...
        if (label) {
            ...
            copyBtn.setAttribute('title', 'Link copied — paste in Instagram');
        }

        if (copyBtn.hasAttribute('data-ig-share')) {
            const profile = copyBtn.getAttribute('data-ig-profile');
            if (profile) {
